...
NTSTATUS scldrDevExtSetBuf(struct ScldrDevExt *dev_ext,const char *buf,ULONG len);
NTSTATUS scldrDevExtSetBufUnsafe(struct ScldrDevExt *dev_ext,const char *buf,ULONG len);
...
/// <summary>
/// 驱动入口
/// Register IRP dispatch routines, create device object and configure for buffered I/O, initialize device extension, and create symlink.
/// </summary>
...
)
{
	...
	//IRP_MJ_WRITE主要响应写请求，实现从用户态将shellcode写入驱动的设备扩展
...
/// <summary>
/// 将shelcode写入缓冲区 IRQL is constrained by the FAST_MUTEX, but dispatch routines will always be called at PASSIVE_LEVEL.
/// </summary>
/// <param name="DeviceObject"></param>
/// <param name="Irp"></param>
/// <returns>STATUS_SUCCESS if successful;STATUS_INVALID_DEVICE_STATE if device extension is null;</returns>
NTSTATUS
scldrWrite(
	__inout PDEVICE_OBJECT DeviceObject,
	__inout PIRP Irp
...
/// IOCTL_kscldr_setbreakpointdisposition: set/clear pre-call breakpoint;断点
/// IOCTL_kscldr_setmaxlength: set shellcode buffer maximum length;设置shellcode缓冲区长度
/// IOCTL_kscldr_callsc: call shellcode at APC_LEVEL with optional offset.调用shellcode apc_level irql
/// </summary>
/// <param name="DeviceObject">DeviceObject</param>
...
exit_scldrDeviceControl:
	if (took_mutex)
	{
		ExReleaseFastMutex(&dev_ext->mutex);//释放快速锁
	}
...
/**
 * Set the buffer of an ScldrDevExt without taking any mutexes (assumes the caller has taken care of synchronization).
 * 
 *
...
/// <summary>
/// 设置设备扩展内存区但是不带任何锁（假定调用者注意到了同步问题）
/// Set the buffer of an ScldrDevExt without taking any mutexes (assumes the caller has taken care of synchronization).
/// </summary>
/// <param name="dev_ext"></param>
/// <param name="buf"></param>
...
)
{
	...
	if ((NULL == dev_ext) || (NULL == buf) || (!len) || (len > dev_ext->max_len))
	{
		nts = STATUS_INVALID_PARAMETER;
		goto exit_scldrDevExtSetBufUnsafe;
	}

	// Dispose of any memory already allocated 释放内存
	if (dev_ext->buf)
	{
		ExFreePoolWithTag(dev_ext->buf, TAG);
		dev_ext->buf = NULL;
